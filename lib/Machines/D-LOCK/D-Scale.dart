...
class DScaleScreen extends StatefulWidget {
  final String username;

  const DScaleScreen({Key? key, required this.username}) : super(key: key);

  @override
  _DScaleScreenState createState() => _DScaleScreenState();
}

class _DScaleScreenState extends State<DScaleScreen> {
  ...
  Widget _buildStoreroomList() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('SUPPLYSCALE MACHINE', style: TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.bold)),
        SizedBox(height: 25),
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) => Padding(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  'D-LOCK',
                  style: GoogleFonts.notoSans(fontSize: 15, color: Colors.white, fontWeight: FontWeight.bold),
                ),
              ),
            ),
          ),
  ...
  Widget _buildActionsPanel() {
    final List<Map<String, dynamic>> actions = [
      ...
      {'label': 'CONFIG', 'icon': Icons.cloud_upload, 'onTap': () => print('Load action')},
    ];
