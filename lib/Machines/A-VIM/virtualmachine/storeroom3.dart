...
import 'package:login_v3_nextgen/Machines/A-VIM/virtualmachine/storeroom3/IssueDispense.dart';
...
class Storeroom3Screen extends StatefulWidget {
  final String username;

  const Storeroom3Screen({Key? key, required this.username}) : super(key: key);

  @override
  _Storeroom3ScreenState createState() => _Storeroom3ScreenState();
}

class _Storeroom3ScreenState extends State<Storeroom3Screen> {
              ...
              child: ElevatedButton(
                onPressed: () {
                  if (index == 2) {
                    Navigator.pushReplacement(
                      context,
                      MaterialPageRoute(builder: (context) => Storeroom3Screen(username: widget.username)),
                    );
                  } else {
                ...
                child: Text(
                  'Storeroom ${index + 1}',
                  style: GoogleFonts.notoSans(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold),
                ),
              ),
            ),
          ),
  ...
  Widget _buildActionsPanel() {
    final List<Map<String, dynamic>> actions = [
      {'label': 'ISSUE/DISPENSE', 'icon': Icons.download, 'onTap': () =>
      Navigator.pushReplacement(context,  MaterialPageRoute(builder: (context) => IssueDispenseScreen(username: widget.username)),)},
      ...
      {'label': 'RECLAIM', 'icon': Icons.store, 'onTap': () => print('Stock action')},
      {'label': 'LOAD', 'icon': Icons.shopping_cart, 'onTap': () => print('Order action')},
      {'label': 'UNLOAD', 'icon': Icons.restore, 'onTap': () => print('Reclaim action')},
      {'label': 'AD HOC ORDER', 'icon': Icons.cloud_upload, 'onTap': () => print('Load action')},
      {'label': 'CHECKED OUT', 'icon': Icons.cloud_download, 'onTap': () => print('Unload action')},
    ];
    ...
    return Container(
      ...
      child: Row(
        children: [
          SizedBox(width: 10),
          Image.asset(
              'assets/images/logobot.png', height: 20),
          SizedBox(width: 10),
          Text(
              appVersion, style: TextStyle(color: Colors.white, fontSize: 12)),
          Spacer(),
          Text(
              getUsaGmtMinus4DateTime(), style: TextStyle(color: Colors.white, fontSize: 12)),
          SizedBox(width: 10),
        ],
      ),
    );
